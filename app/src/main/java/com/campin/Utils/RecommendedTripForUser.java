...
public class RecommendedTripForUser {
    ...
    public static void recommendedTripForUser(final User user, final Model.GetAllTripsListener listener){
        ...
        Model.instance().getAllTripAsynch(new Model.GetAllTripsListener() {
            @Override
            public void onComplete(final List<Trip> tripsList, int currentMaxKey) {
                Model.instance().getTripsUserBelongs(new Model.getTripsUserBelongsListener() {
                    ...
                    public void onComplete(ArrayList<PlannedTrip> plannedTripList, int currentMaxKey) {
                        for (PlannedTrip plannedTrip : plannedTripList){
                            tripsList.remove(Model.instance().getTripById(String.valueOf(plannedTrip.getTripId())));
                        }
                ...
                public void onComplete(User user) {
                    ...
                    if (allUsers.size() == usersId.size()) {
                        ...
                        Model.instance().getAllTripAsynch(new Model.GetAllTripsListener() {
                            @Override
                            public void onComplete(final List<Trip> tripsList, int currentMaxKey) {
                                Model.instance().getTripsUsersBelongs(usersId, new Model.getTripsUserBelongsListener() {
                                    @Override
                                    public void onComplete(ArrayList<PlannedTrip> plannedTripList, int currentMaxKey) {
                                        for (PlannedTrip plannedTrip : plannedTripList){
                                            tripsList.remove(Model.instance().getTripById(String.valueOf(plannedTrip.getTripId())));
